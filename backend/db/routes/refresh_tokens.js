...
export async function insertRefreshToken(token, id_usuario, device, tenant_id) {
    ...
    const [rows] = await pool.query(query, [id_usuario, token, expirationDay, device, tenant_id])

    if (rows.affectedRows > 0) {
        await registerConsult('INSERT', 'refresh_tokens', query)
        return true
    }
    return false
}
...
export async function revokeRefreshToken(id_user, device, tenant_id) {
    ...
    if (rows.affectedRows > 0) {
        await registerConsult('UPDATE', 'refresh_tokens', query)
        return rows[0]
    }
    return true
}
...
export async function verifyRefreshToken(token, device, tenant_id) {
    ...
    await registerConsult('SELECT', 'refresh_tokens', query)
    if (!rows.length) {
        return false
    }
    return true
